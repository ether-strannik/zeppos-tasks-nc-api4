...
export class ScreenBoard {
  ...
  build() {
    this.group = hmUI.createWidget(hmUI.widget.VIEW_CONTAINER, {
      x: 0,
      y: 0,
      w: DeviceInfo.width,
      h: DeviceInfo.height,
    ...
    if(!this.renderer.hasBackspace) {
      ...
      this.backspaceView.addEventListener(event.CLICK_UP, this.doBackspace)
    }
 ...
 get visible() {
  ...
  set visible(v) {
    ...
    // hmApp.setLayerY(0) removed - not available in API 3.0
  }
