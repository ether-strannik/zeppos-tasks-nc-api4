...
export class ListScreen {
	...
	threeActionBar(items) {
		// For narrow screens, fall back to rows
		if(WIDGET_WIDTH < 300) {
			const row = this.row(items[0]);
			this.row(items[1]);
			this.row(items[2]);
		...
		const buttonSize = this.baseRowHeight;
		const buttonGap = 8;
		// Two buttons on the right: [voice] [new]
		const buttonsWidth = (buttonSize * 2) + buttonGap;
		const firstWidth = WIDGET_WIDTH - buttonsWidth - buttonGap;

		const iconPos = Math.floor((buttonSize - ICON_SIZE_SMALL) / 2);

		// First circular button (middle - e.g., voice)
		const button1 = this.card({
			width: buttonSize,
			height: buttonSize,
			offsetX: firstWidth + buttonGap,
			radius: Math.floor(buttonSize / 2),
			color: items[1].color || this.accentColor,
			dontChangePosY: true
		});

		button1.group.createWidget(hmUI.widget.IMG, {
			x: iconPos,
			y: iconPos,
			src: items[1].icon
		});
		...
		if(items[1].callback) {
			button1.getEvents().ontouch = items[1].callback;
		}

		// Second circular button (right - e.g., new)
		const button2 = this.card({
			width: buttonSize,
			height: buttonSize,
			offsetX: firstWidth + buttonGap + buttonSize + buttonGap,
			radius: Math.floor(buttonSize / 2),
			color: items[2].color || this.accentColor,
			dontChangePosY: true
		});

		button2.group.createWidget(hmUI.widget.IMG, {
			x: iconPos,
			y: iconPos,
			src: items[2].icon
		});
		...
		if(items[2].callback) {
			button2.getEvents().ontouch = items[2].callback;
		}

		// Main row (left - list title)
		const row = this.row({
			...items[0],
			card: {
				width: firstWidth,
